...
#######################################
	...
	executable = '/afs/cern.ch/user/c/cbasile/CMSSW-10-6-20-Analysis/src/BParkNANO/B0toPsi2SK0s/X3872App'
	...
	while (len(inputfiles) > 0):
		...
		for line in range(min(opt.nfileperjob,len(inputfiles))):
			...
			if(opt.eos!=''):	
				outputfile.write('cp '+rootoutputfile+'_tree.root /eos/user/c/cbasile/HLTapplyPsi2S/'+output+'_'+str(ijob)+'_tree.root\n')
				outputfile.write('cp '+rootoutputfile+'_histo.root /eos/user/c/cbasile/HLTapplyPsi2S/'+output+'_'+str(ijob)+'_histo.root\n')
