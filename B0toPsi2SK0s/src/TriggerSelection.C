...
void TriggerSelection::Loop() {
   ...
   int Ncandidates_JPsi = 0, Ncandidates_Psi2S = 0, Ncandidates_B0 = 0, TOTcand_JPsi = 0, TOTcand_B0 = 0;
   ...
   bool isTriggerON, isTrkQC, isSel_MuMu, isSel_Trk, isSel_Pim, isSel_Pip;
	bool isMCmatched_JPsi, isMCmatched_Pi1, isMCmatched_Pi2, isMCmatched_Rho, isMCmatched_Psi2S, isMCmatched_K0s, isMCmatched_B0;	
	bool isSGNregion_Psi2S = true, isSGNregion_K0s = true;
   ...
   float Mlow, Mhigh;
	...
	Mlow = 3.6; Mhigh = 3.75;
	TH1F h_PREfit_SGN_Psi2S_M("PREfit_SGN_Psi2S_M", "X(3872)", nbins, Mlow, Mhigh);	
	TH1F h_PREfit_BKG_Psi2S_M("PREfit_BKG_Psi2S_M", "X(3872)", nbins, Mlow, Mhigh);	
	TH1F h_TOTfit_SGN_Psi2S_M("TOTfit_SGN_Psi2S_M", "X(3872)", nbins, Mlow, Mhigh);	
	TH1F h_TOTfit_BKG_Psi2S_M("TOTfit_BKG_Psi2S_M", "X(3872)", nbins, Mlow, Mhigh);	
	...
	// SIGNAL HISTO TO FIT
   nbins = 100;
	TH1F h_SGN_Psi2S_M("SGN_Psi2S_M", "\\Psi(2S)", nbins, 3.5, 3.85);	
	nbins = 120;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      MCtruthMatching();
		...
		Ncandidates_JPsi = 0, Ncandidates_Psi2S = 0, Ncandidates_B0 = 0 ; 
	...
	//==> MC truth matching
			...
			//...X(3872)
			isMCmatched_Psi2S = isMCmatched_Rho && isMCmatched_JPsi; 
			ToCountX = ToCountRho || ToCountJPsi;
			if( isMCmatched_Psi2S && ToCountX){ 
				Nmatch_X++; Ncandidates_Psi2S++; 
				h_PREfit_SGN_Psi2S_M.Fill(MuMuPiPi_M);
				h_TOTfit_SGN_Psi2S_M.Fill(B0_finalFit_X_mass[b]);
			}else if (ToCountX){
				Ncandidates_Psi2S++;
				h_PREfit_BKG_Psi2S_M.Fill(MuMuPiPi_M);
				h_TOTfit_BKG_Psi2S_M.Fill(B0_finalFit_X_mass[b]);
			}  
			//isSGNregion_Psi2S = ( B0_finalFit_X_mass[b] > MX_nearLeft ) && ( B0_finalFit_X_mass[b] < MX_nearRight );
			...
			if (isMCmatched_JPsi && isMCmatched_Rho && isMCmatched_K0s){ 
				h_SGN_Psi2S_M.Fill(B0_finalFit_X_mass[b]);	
				h_SGN_K0s_M.Fill(B0_K0s_nmcFitted_mass[b]);	
				h_SGN_B0_M.Fill(B0_finalFit_mass[b]);	
				if(isSGNregion_Psi2S && isSGNregion_K0s){
					// TTree variables 
					B0_SGN_idx = b;
					Nmatch_B0++;
   ...
   std::cout << "Events with at least 1 B0 candidate:" << N_B0recoEV << std::endl;
   std::cout << "Rho matching MC thruth:" << Nmatch_Rho << std::endl;  
   std::cout << "Psi(2S) matching MC thruth:" << Nmatch_X << std::endl;
   ...
   std::cout << "B0 matching MC thruth:" << Nmatch_B0 << std::endl;
	...
	h_PREfit_SGN_Psi2S_M.Write();
	h_TOTfit_SGN_Psi2S_M.Write();
	h_PREfit_BKG_Psi2S_M.Write();
	h_TOTfit_BKG_Psi2S_M.Write();
	...
	std::cout << "OUTPUT histo written on file ---> ./plots/TriggerSelection.root" << std::endl;
...
void TriggerSelection::GenPartFillP4(){
   ...
   for (UInt_t g = 0; g < nGenPart; g++){
      if( (GenPart_pdgId[g] ==  isMum) &&  (GenPart_pdgId[GenPart_genPartIdxMother[g]] == isJPsi) &&
			 (GenPart_pdgId[GenPart_genPartIdxMother[GenPart_genPartIdxMother[g]]] == isPsi2S)) MumIdx = g;
      if( (GenPart_pdgId[g] == -isMum) &&  (GenPart_pdgId[GenPart_genPartIdxMother[g]] == isJPsi) &&
			 (GenPart_pdgId[GenPart_genPartIdxMother[GenPart_genPartIdxMother[g]]] == isPsi2S)) MupIdx = g;
      if( (GenPart_pdgId[g] == -isPip) &&  
			 (GenPart_pdgId[GenPart_genPartIdxMother[g]] == isPsi2S) ) PimIdx = g;
      if( (GenPart_pdgId[g] ==  isPip) &&  
			 (GenPart_pdgId[GenPart_genPartIdxMother[g]] == isPsi2S) ) PipIdx = g;
