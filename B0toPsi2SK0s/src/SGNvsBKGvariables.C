...
SGNvsBKGvariables::SGNvsBKGvariables(TTree *tree) : B0toX3872K0s(tree){ 
	...
	inFilePath_ = "/afs/cern.ch/user/c/cbasile/CMSSW-10-6-20-Analysis/src/BParkNANO/B0toPsi2SK0s/results/B0_SGNvsBKG_anlaysis.root";
	outFilePath_ = "/afs/cern.ch/user/c/cbasile/CMSSW-10-6-20-Analysis/src/BParkNANO/B0toPsi2SK0s/results/TMVAinputs.root";
...
//---- HISTOGRAMS ----//
	...
	Mlow = 3.7, Mhigh = 4.05;	
	TH1F h_SGN_Psi2S_M("SGN_Psi2S_M", "", nbins, Mlow, Mhigh );
	...
	TH1F h_SGN_Psi2S_pT("SGN_Psi2S_pT", "", nbins, 0.4, 1.);
	TH1F h_BKG_Psi2S_pT("BKG_Psi2S_pT", "", nbins, 0.4, 1.);
   ...
   for (Long64_t jcand = 0; jcand < NB0cand; jcand++) {
	...
	// ==================== SIGNAL B0 ====================//
		if (!(B0_SGN_idx < 0)){
			...
			//... X(3872)
			h_SGN_Psi2S_M.Fill(P4_Reco_Psi2S.M());
			h_SGN_Psi2S_pT.Fill(P4_Reco_Psi2S.Pt()/B0_finalFit_pt[B0_SGN_idx]);
	...
	// ==================== BACKGROUND B0 ====================//
		...
		for (UInt_t bb = 0; bb < nBKG_B0; bb++){
			...
			//... X(3872)
			if (!B0_BKG_isTrueJPsi[bb] || !B0_BKG_isTrueRho[bb]) h_BKG_Psi2S_pT.Fill(P4_Reco_Psi2S.Pt()/B0_finalFit_pt[Bidx]); 
	...
	h_SGN_Rho_pT.Write();
	h_BKG_Rho_pT.Write();
	h_SGN_Psi2S_pT.Write();
	h_BKG_Psi2S_pT.Write();
	h_SGN_K0s_pT.Write();
	h_BKG_K0s_pT.Write();
...
int SGNvsBKGvariables::RecoPartFillP4(const int Bidx){
	...
	//... X(3872)
	P4_Reco_Psi2S = P4_Reco_Rho + P4_Reco_JPsi; P4_Reco_Psi2S.SetM(B0_finalFit_X_mass[Bidx]);
	//std::cout << "Psi2S ... " << P4_Reco_Psi2S.Pt() << "\t fit " << B0_finalFit_X_mass[Bidx] << std::endl;
	//... K0s track
