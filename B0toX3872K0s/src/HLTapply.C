...
#ifdef UNBLIND
else outFileTreePath_.Append("UNBLIND_");
#else
...
void HLTapply::Loop() {
   ...
   int Ncandidates_JPsi = 0, Ncandidates_Rho = 0, Ncandidates_X3872 = 0, Ncandidates_K0s = 0, Ncandidates_B0 = 0;
	int TOTcand_JPsi = 0, TOTcand_B0 = 0, TOTsavedB0 = 0 ;
   int NTriggeredEv = 0, NTrigSelEV = 0, NMuMu_Pt25 = 0;
	int TriggeredTrkID;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      ToCountJPsi = true, ToCountRho = true, ToCountX = true, ToCountK0s = true; 
		...
		for (UInt_t b = 0; b < nB0; b++){
			...
			if ((P4_Reco_Mu1 + P4_Reco_Mu2).Pt() > 25.)NMuMu_Pt25++;
	...
	//==> TRIGGER SELECTION
      	// --> TRIGGER SELECTION on muons
			...
			if (B0_sidebands && X_SgnRegion && K0s_SgnRegion){ 
				...
				P4_Reco_K0sPi1 = P4_Reco_K0s + P4_Reco_Pi1; 
				P4_Reco_K0sPi2 = P4_Reco_K0s + P4_Reco_Pi2; 
				...
				M_K0sPi1 = P4_Reco_K0sPi1.M();
				M_K0sPi2 = P4_Reco_K0sPi2.M();
   ...
   std::cout << "PT(MuMu)>25 GeV number of events       " << NMuMu_Pt25<< std::endl;
   ...
   std::cout << "Number of B0 saved in the output :     " << TOTsavedB0 << "  ( " << (double)TOTsavedB0/ TOTcand_B0 << " )" << std::endl;
...
#ifdef UNBLIND
	TString TreeName = "B0_X3872signal";
#else
