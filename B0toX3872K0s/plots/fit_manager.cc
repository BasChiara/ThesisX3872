...
TFile* open_file(TString file = ""){
	...
	if (file == "BKG") root_file = "/afs/cern.ch/user/c/cbasile/CMSSW-10-6-20-Analysis/src/BParkNANO/B0toX3872K0s/data/merged/BKG_MergData17.root";	
...
TString CategoryLegend(const TString& category){
  ...
  Leg_entry["BKG_JPsi"] = "B_{0} BAKGROUND";

  Leg_entry["SGN_Rho"] = "SIGNAL #rho (770)";
  Leg_entry["BKG_Rho"] = "BACKGROUND #rho (770)";

  Leg_entry["SGN_K0s"] = "SIGNAL K^{0}_{s}";
  Leg_entry["BKG_K0s"] = "BACKGROUND";

  Leg_entry["SGN_X3872"] = "SIGNAL X(3872)"; 
  Leg_entry["BKG_X3872"] = "J/#Psi - BKG";

  Leg_entry["SGN_B0"] = "B_{0} SIGNAL";
...
int Fit_sidebandsB0(TString model){
	...
	// LOAD TREE 
	TFile* input_file = open_file("BKG");
	TTree* TreeBKG = (TTree*)input_file->Get("B0sidebands");
	
	// LOAD MVA-CUT OUTPUT
	TFile* RootFileCUT = open_file("MVA"); 
	TTree* TreeBKGcut = (TTree*)RootFileCUT->Get("OutTreeBKG");
	

	// GET HISTO 
	int Nbins = 75;
	double Mlow = 5.0, Mhigh = 5.6;
	TH1F* h_BKG_B0 = new TH1F("BKG_B0", "", Nbins, Mlow, Mhigh); 
	TreeBKG->Draw("M_B0>>BKG_B0");
	TH1F* h_BKG_B0cut = new TH1F("BKG_B0cut", "", Nbins, Mlow, Mhigh); 
	TreeBKGcut->Draw("BKG_MB0>>BKG_B0cut");
	//TH1F*  h_BKG_B0cut = (TH1F*)RootFileCUT->Get("BKG_MB0"); 


	std::cout << " ENTRIES " << h_BKG_B0->GetEntries() << std::endl;	
	histo_SetUp(h_BKG_B0, "BKG_B0", "M(B_0) \\ GeV", false, false);
	histo_SetUp(h_BKG_B0cut, "BKG_B0_pstcut", "M(B_0) \\ GeV", false, false);


	// B0 mass RANGE
	RooRealVar B0m("M_B0", "B0 mass sidebands", 5.07947, 5.47934);

	B0m.setRange("SB1", 5.07947, 5.17944); // SIDEBAND 1
	B0m.setRange("SB2", 5.37937, 5.47934); // SIDEBAND 2
	B0m.setRange("FULLregion", 5.07947, 5.47934); // FULLregion RANGE 
	B0m.setRange("SGNregion", 5.17944, 5.37937); // SGNregion RANGE 

	RooDataSet B0_mass ("B0_mass","B0_mass", B0m, Import(*TreeBKG)) ;
	...
	RooRealVar b0("b0", "",  -200,  -10000.,  -100.);
	RooRealVar b1("b1", "",  0.2,     10.,  500.);
	RooRealVar b2("b2", "",  0.5,  -10.,  10.);
	RooRealVar b3("b3", "",  0.2,  -100.,  100.);
	RooChebychev Cheb("Cheb", "", B0m, RooArgSet(b0, b1, b2) );
	RooPolynomial Line("Line", "", B0m, RooArgList(b0, b1));
	RooProdPdf BKGline("BKGbase", "BKGbase", Line); 

	// Create my PDF
	RooRealVar A("A", "", 10.0, 1.0 , 30.);
	RooRealVar Flex("Flex", "", 4., 3. , 5.2);
...
int Fit_mass_X(TString func = "GAUS"){
   ...
   result.Print(std::cout);
	...
	if (func == "GAUS"){
		...
		fitSubFunc_G2->SetLineColor(kOrange); fitSubFunc_G2->SetLineWidth(3);
	}
	...
	float SIGMA_ERR = 4./SIGMA * sqrt( (*(FitParams+2) * wG1)*(*(FitParams+2) * wG1) * (wG1 * *(FitParErr+2))*(wG1 * *(FitParErr+2))  + 
												  (*(FitParams+5) * wG2)*(*(FitParams+5) * wG2) * (wG2 * *(FitParErr+5))*(wG2 * *(FitParErr+5))); 
	...
	histo_SetUp(h, "SGN_X3872", "M(J/#Psi #rho) GeV", false, false);
	h->SetTitle("");

	h->Draw(); fitSubFunc_G1->Draw("same"); fitSubFunc_G2->Draw("same");
	fitFunc->Draw("same");
	NearR->Draw("same"); NearL->Draw("same");
	legend->AddEntry(h, CategoryLegend("SGN_X3872"));
	legend->AddEntry("fitFunc", "FIT " ,"l");
	legend->AddEntry("fitSubFunc_G1", "Gaussian-1" ,"l");
...
int Fit_mass_K0s(TString func = "GAUS"){
   ...
   result.Print(std::cout);
	...
	if (func == "GAUS"){
		fitSubFunc_G1 = new TF1("fitSubFunc_G1","gaus",Mlow, Mhigh); // [0] norm [1] mean [2] sigma
		fitSubFunc_G1->SetParameters(*FitParams, *(FitParams+1), *(FitParams+2));
		fitSubFunc_G1->SetLineWidth(3); fitSubFunc_G1->SetLineColor(kBlue);
		fitSubFunc_G2 = new TF1("fitSubFunc_G2","gaus",Mlow, Mhigh); // [0] norm [1] mean [2] sigma
		fitSubFunc_G2->SetParameters(*(FitParams+3), *(FitParams+4), *(FitParams+5));
		fitSubFunc_G2->SetLineWidth(3); fitSubFunc_G2->SetLineColor(kOrange);
	}
	// SIGNAL REGION  
	...
	float SIGMA_ERR = 4./SIGMA * sqrt( (*(FitParams+2) * wG1)*(*(FitParams+2) * wG1) * (wG1 * *(FitParErr+2))*(wG1 * *(FitParErr+2))  + 
												  (*(FitParams+5) * wG2)*(*(FitParams+5) * wG2) * (wG2 * *(FitParErr+5))*(wG2 * *(FitParErr+5))); 
	std::cout << "\nMEAN " << meanKm << "\tSIGMA " << SIGMA << " +/- "  << SIGMA_ERR << std::endl;
	float nSIG = 4;
	...
	histo_SetUp(h, "SGN_K0s", "M(K_{s}^{0}) GeV", false, false);
	h->SetTitle("");
	h->Draw();
	if(func == "GAUS+CB"){
	...
	fitFunc->Draw("same");
	NearR->Draw("same"); NearL->Draw("same");
	legend->AddEntry(h, CategoryLegend("SGN_K0s"));
	legend->AddEntry("fitFunc", "FIT " ,"l");
	legend->AddEntry("fitSubFunc_G1", "Gaussian-1" ,"l");
	legend->AddEntry("fitSubFunc_G2", "Gaussian-2" ,"l");
	legend->Draw();
...
int Fit_mass_B0(std::string func = "GAUS+CB"){
	...
	// ... gauss
	fitter.Config().ParSettings(0).SetLimits(30, 150);   // ampl
	fitter.Config().ParSettings(1).SetLimits(5.27,5.28); // mean 
	//fitter.Config().ParSettings(1).Fix();
	fitter.Config().ParSettings(2).SetLimits(0.0, 0.05); // sigma
	// ... crystal ball
	fitter.Config().ParSettings(3).SetLimits(0, 80);   // ampl
	fitter.Config().ParSettings(4).SetLimits(5.27,5.28); // mean
	//fitter.Config().ParSettings(4).Fix();
	fitter.Config().ParSettings(5).SetLimits(0.0, 0.1);  // sigma
	fitter.Config().ParSettings(6).SetLimits(0., 50.);  // N
   ...
   result.Print(std::cout);
	...
	fitSubFunc_G = new TF1("fitSubFunc_G","gaus",Mlow, Mhigh); // [0] norm [1] mean [2] sigma
	fitSubFunc_G->SetParameters(*FitParams, *(FitParams+1), *(FitParams+2));
	fitSubFunc_G->SetLineColor(kGreen); fitSubFunc_G->SetLineWidth(3);
	...
	float SIGMA_ERR = 4./SIGMA * sqrt( (*(FitParams+2) * wG )*(*(FitParams+2) * wG ) * (wG  * *(FitParErr+2))*(wG  * *(FitParErr+2))  + 
												  (*(FitParams+5) * wCB)*(*(FitParams+5) * wCB) * (wCB * *(FitParErr+5))*(wCB * *(FitParErr+5))); 
	std::cout << "\nMEAN " << meanBm << "\tSIGMA " << SIGMA << " +/- "  << SIGMA_ERR << std::endl;
	...
	//LEGEND
	auto legend = new TLegend(0.18,0.75,.41,.89);
	legend->SetBorderSize(0);
	gStyle->SetLegendTextSize(0.03);
	...
	TCanvas* c1 = new TCanvas("c1","canvas", 1024, 1024);
	TString png_name_log = pngName(histo_name + "_FitLOG" + func, "");
	TString pdf_name_log = pdfName(histo_name + "_FitLOG" + func, "");

	histo_SetUp(h, "SGN_B0", "M(B_{0}) GeV",  false, false);
	h->SetTitle("");
	...
	legend->AddEntry("fitFunc", "FIT" ,"l");
	legend->AddEntry("fitSubFunc_G", "Gaussian" ,"l");
